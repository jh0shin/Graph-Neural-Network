...
if args.platform == 'pyg':
    ...
    with torch.autograd.profiler.emit_nvtx() as prof:
        for _ in range(_EPOCH):      
            optimizer.zero_grad()
            with profiler.record_function("forward"): out = model(data.x, data.adj_t)[train_idx]
            with profiler.record_function("loss"): loss = F.nll_loss(out, data.y.squeeze(1)[train_idx])
            with profiler.record_function("backward"): loss.backward()
            with profiler.record_function("optimizer.step"): optimizer.step()
...
if args.platform == 'dgl':
    ...
    with torch.autograd.profiler.emit_nvtx() as prof:
        for _ in range(_EPOCH):
            with profiler.record_function("forward"): out = model(data, features)
            with profiler.record_function("loss"): loss = F.nll_loss(out[train_idx], label.squeeze(1)[train_idx])
