...
if args.platform == 'dgl':
    ...
    model = DglSequential(
        ...
        GraphConv2(args.hidden_channel, args.hidden_channel, timer, activation=F.relu),
        GraphConv2(args.hidden_channel, dataset.num_classes, timer, activation=F.log_softmax),
    ).to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=0.01)
    evaluator = Evaluator(name=args.dataset)
